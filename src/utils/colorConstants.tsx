// ---------------------------- //
// ---------------------------- //
// Project general LIGHT colors
// ---------------------------- //
// ---------------------------- //
export const LIGHT_PRIMARY_COLOR = '#417cde'
export const LIGHT_SECONDARY_COLOR = 'red'

// Text colors
export const LIGHT_TEXT_WITH_PRIMARY = '#ffffff'
export const LIGHT_TEXT_WITH_SECONDARY = ''
export const LIGHT_TEXT_DISABLED = '#999'

// Utilities colors
export const LIGHT_LIGHT_BORDER = '#eaeaea'
export const LIGHT_DISABLED_COLOR = '#f2f2f2'
export const LIGHT_DISABLED_BORDER = '#c5c5c5'

// Defaults colors
export const LIGHT_DEFAULT_BACKGROUND = '#fff'
export const LIGHT_DEFAULT_HOLLOW_BACKGROUND = '#f3f5fd'
export const LIGHT_DEFAULT_TEXT = '#8792a7'
export const LIGHT_DEFAULT_TEXT_ACTIVE = '#999'

// Sidebar colors
export const LIGHT_SIDEBAR_BACKGROUND = '#fff'
export const LIGHT_SIDEBAR_HOVER = '#f1f3f7'


// ---------------------------- //
// ---------------------------- //
// Project general DARK colors
// ---------------------------- //
// ---------------------------- //
export const DARK_PRIMARY_COLOR = '#1a3b75'
export const DARK_SECONDARY_COLOR = 'red'

// Text colors
export const DARK_TEXT_WITH_PRIMARY = '#ffffff'
export const DARK_TEXT_WITH_SECONDARY = ''
export const DARK_TEXT_DISABLED = '#999'

// Utilities colors
export const DARK_LIGHT_BORDER = '#0b1931'
export const DARK_DISABLED_COLOR = '#f2f2f2'
export const DARK_DISABLED_BORDER = '#c5c5c5'

// Defaults colors
export const DARK_DEFAULT_BACKGROUND = '#141729'
export const DARK_DEFAULT_HOLLOW_BACKGROUND = '#0a111e'
export const DARK_DEFAULT_TEXT = '#a6b2b7'
export const DARK_DEFAULT_TEXT_ACTIVE = '#fffffff'

// Sidebar colors
export const DARK_SIDEBAR_BACKGROUND = '#0e1627'
export const DARK_SIDEBAR_HOVER = '#16233a'